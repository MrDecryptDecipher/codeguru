...
export class WindowHelper {
  ...
  constructor(appState: AppState) {
    ...
    if (windowConfig.position) {
      this.windowPosition = windowConfig.position;
    }
    if (windowConfig.size) {
      this.windowSize = windowConfig.size;
    }
  }
  ...
  public createWindow(): void {
    ...
    const windowSettings: Electron.BrowserWindowConstructorOptions = {
      ...
      opacity: windowConfig.opacity || 0.95
    }
  ...
  public moveToDisplay(displayId: number): void {
    ...
    if (displayId < 0 || displayId >= displays.length) {
      console.warn(`[WindowHelper] Invalid display ID: ${displayId}`);
      return;
    }
  ...
  public setOpacity(opacity: number): void {
    ...
    this.configManager.updateSection('window', { opacity: clampedOpacity });
  }
  ...
  public saveWindowState(): void {
    ...
    const bounds = this.mainWindow.getBounds();
    this.configManager.updateSection('window', {
