...
const ModelSelector: React.FC<ModelSelectorProps> = ({ onModelChange, onChatOpen }) => {
  ...
  const [selectedProvider, setSelectedProvider] = useState<"ollama" | "gemini" | "openrouter">("openrouter");
  ...
  const loadCurrentConfig = async () => {
    try {
      ...
      if (config.isOllama) {
        ...
        // If models are already loaded, we don't need API key
        if (availableOpenRouterModels.length > 0) {
          // Models are already configured, no need for API key input
        }
      }
    } catch (error) {
  ...
  const handleProviderSwitch = async () => {
    try {
      ...
      if (selectedProvider === 'ollama') {
        ...
        // Try to load models first if not already loaded
        if (availableOpenRouterModels.length === 0) {
          await loadOpenRouterModels();
        }
        
        // If models are loaded, we can switch (API key will be read from config file if not provided)
        if (availableOpenRouterModels.length > 0 && openRouterApiKey) {
          result = await window.electronAPI.switchToOpenRouter(
            openRouterApiKey, 
            availableOpenRouterModels
          );
        } else {
          // No models available - need to read from config file
          setConnectionStatus('error');
          setErrorMessage('No OpenRouter models found. Please ensure openrouter.txt file exists with models list.');
          return;
        }
      } else {
      ...
      if (result.success) {
        ...
        const modelName = selectedProvider === 'ollama' ? selectedOllamaModel : 
                         selectedProvider === 'openrouter' ? `OpenRouter (${availableOpenRouterModels.length} models)` :
                         'gemini-2.0-flash';
  ...
  return (
    <div className="p-4 bg-white/20 backdrop-blur-md rounded-lg border border-white/30 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              <label className="text-xs font-medium text-gray-700">Models ({availableOpenRouterModels.length})</label>
              <button
            ...
            {availableOpenRouterModels.length > 0 ? (
              <div className="text-xs text-gray-600 bg-white/40 p-2 rounded max-h-32 overflow-y-auto">
                {availableOpenRouterModels.map((model, idx) => (
                  <div key={idx} className="py-1">{model}</div>
                ))}
              </div>
            ) : (
              <div className="text-xs text-gray-600 bg-yellow-100/60 p-2 rounded">
                No OpenRouter models loaded. Models will be loaded from openrouter.txt file.
              </div>
            )}
          </div>
        </div>
      ...
      <div className="text-xs text-gray-600 space-y-1">
        <div>🌐 <strong>OpenRouter:</strong> Orchestrated free models, no limitations</div>
        <div>☁️ <strong>Gemini:</strong> Fast, cloud-based, requires API key</div>
